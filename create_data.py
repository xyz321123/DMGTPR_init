...
class DomainAdaptationAmazon:
    ...
    def run_non_kfold(self):
        ...
        dev_data = self.process_data(xs_test[:len(xt_test)], ys_test[:len(xt_test)], np.zeros_like(yt_test[:self.batch_size]), vocab=vocab)
        ...
        number_of_aux_labels = len(np.unique(ys))
        ...
        return vocab, number_of_labels, number_of_aux_labels, iterators, other_meta_data # empty dict for other_meta_data
...
class EncodedBiasInBios(): # 重点
    def __init__(self, dataset_name:str,**params):
        self.dataset_name = dataset_name.lower() # encoded_bias_in_bios
        self.batch_size = params['batch_size'] # 2000
        self.fairness_iterator = params['fairness_iterator']
        ...
        for d in data_location:
            try:
                ...
                # 255710个训练集，39369个验证集，98344个测试集，每条样本为[性别(2),职业(28),文本语句(n)]
                ...
                # Bert的维度为[1,768]的整篇文本的语义表示(cls)，数量与上方相同
    ...
    def run(self):
        ...
        number_of_aux_labels = len(np.unique(train_s))
        # 变成64维
        pca = PCA(n_components=64)
        self.train_cls = pca.fit_transform(self.train_cls)
        self.dev_cls = pca.fit_transform(self.dev_cls)
        self.test_cls = pca.fit_transform(self.test_cls)

        train_X, train_y, train_s = np.asarray(self.train_cls)[0:255680], np.asarray(train_y)[0:255680], np.asarray(train_s)[0:255680] # 把样本数量削减到能被320整除
        dev_X, dev_y, dev_s = np.asarray(self.dev_cls)[0:39360], np.asarray(dev_y)[0:39360], np.asarray(dev_s)[0:39360] # 把样本数量削减到能被320整除
        test_X, test_y, test_s = np.asarray(self.test_cls)[0:98240], np.asarray(test_y)[0:98240], np.asarray(test_s)[0:98240] # 把样本数量削减到能被320整除
...
def generate_data_iterators(dataset_name:str, **kwargs): # 重点
    ...
    if "amazon" in dataset_name.lower():
        ...
        vocab, number_of_labels, number_of_aux_labels, iterators, other_meta_data = dataset_creator.run()
    elif dataset_name.lower() == 'encoded_bias_in_bios': # 重点
        dataset_creator = EncodedBiasInBios(dataset_name=dataset_name, **kwargs)
        vocab, number_of_labels, number_of_aux_labels, iterators, other_meta_data = dataset_creator.run()
        # iterators都是[1,768]维的整段文本的语义表示向量
    else:
...
def create_fairness_data(train_X, train_y, train_s, dev_X, dev_y, dev_s, process_data, vocab, method):
    ...
    # assert dev_X.shape[0] >= no_examples_to_sample 明明是小于等于
